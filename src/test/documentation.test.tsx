...
describe('Step 4 documentation generation', () => {
  ...
  it('renders the generation form and complete structured draft review flow', async () => {
    ...
    expect(screen.getByText('Current findings')).toBeInTheDocument()
    expect((screen.getAllByText('Patient context')).length).toBeGreaterThan(0)
    expect(screen.queryByRole('heading', { name: 'Clinical context' })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: /Discharge Summary/ }))
    expect(screen.getByRole('button', { name: /Discharge Summary/ })).toHaveClass('selected')
    fireEvent.click(screen.getByRole('button', { name: /SOAP Note/ }))
    expect(screen.getByRole('button', { name: /SOAP Note/ })).toHaveClass('selected')
    fireEvent.change(screen.getByRole('textbox', { name: /Instructions for this draft/ }), { target: { value: 'Emphasize the current medication plan.' } })
    expect(screen.getByRole('textbox', { name: /Instructions for this draft/ })).toHaveValue('Emphasize the current medication plan.')
    ...
    expect(screen.getByText('Clinical conflict')).toBeInTheDocument()
    ...
    expect(screen.queryByText('Approved clinical information')).not.toBeInTheDocument()
    expect(screen.queryByText(/records added/)).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: /Add to patient memory/ }))
