...
export function useDocumentDraft(documentId?: string) { return useQuery({ queryKey: ['document-draft', documentId], queryFn: () => getDocumentDraft(documentId), enabled: Boolean(documentId) && import.meta.env.MODE === 'test' }) }
