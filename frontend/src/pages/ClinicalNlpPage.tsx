...
export function ClinicalNlpPage() {
  ...
  const filteredEvents = useMemo(() => {
    const source = showAllFindings ? events : [...reviewRequiredEvents, ...addedEvents]
  ...
  const actionPending = writeMemory.isPending
  ...
  useEffect(() => {
    if (nlpProcess.data || nlpProcess.isPending || nlpProcess.isError || !step1Output) return
    if (step1Output.processing_status !== 'complete' && import.meta.env.MODE !== 'test') return
    nlpProcess.mutate({
      document_id: step1Output.document_id,
      patient_id: step1Output.patient_id,
      encounter_id: step1Output.encounter_id,
      step1_output: step1Output,
    })
  }, [step1Output, nlpProcess.data, nlpProcess.isPending, nlpProcess.isError])
  ...
  const openInterpretationEditor = (event: ClinicalEvent) => {
    setInterpretationInput(interpretationCorrections[event.event_local_id] ?? abbreviationOf(event).resolved_value ?? event.processed_text)
    setPendingInterpretation(event)
  }

  const useSuggestion = (event: ClinicalEvent) => {
    const suggestion = abbreviationOf(event).resolved_value
  ...
  const saveInterpretation = () => {
    ...
    if (abbreviationOf(pendingInterpretation).was_ambiguous) setAbbreviationResolutions((current) => ({ ...current, [pendingInterpretation.event_local_id]: 'corrected' }))
  ...
  const correctedEventForWrite = (event: ClinicalEvent): ClinicalEvent => {
    ...
    return { ...event, processed_text: correction, ambiguous_abbreviation_resolved: { ...abbreviationOf(event), resolved_value: correction, was_ambiguous: false }, medication_attributes: event.medication_attributes?.frequency ? { ...event.medication_attributes, frequency: correction } : event.medication_attributes }
  }
  ...
  const confirmReviewAction = () => {
    ...
    if (pendingAction.type === 'add') {
      ...
      // A Step 2 finding is not a memory event yet; rejecting it is a local physician review decision.
      setResolutions((current) => ({ ...current, [pendingAction.event.event_local_id]: 'rejected' }))
...
function SafetyCheck({ ready, pendingAbbreviations, pendingFindings }: { ready: boolean; pendingAbbreviations: number; pendingFindings: number }) {
  return <SectionCard title="Safety check" eyebrow="MEMORY WRITE SAFEGUARD" action={ready ? <span className="verified-heading"><CheckIcon /> Ready</span> : <span className="high-risk-label"><AlertIcon /> Review required</span>} className={ready ? 'safety-check-card safety-ready' : 'safety-check-card'}><p>{ready ? 'All required abbreviation and clinical finding reviews are complete. Findings may be reviewed before patient memory is opened.' : `${pendingAbbreviations} abbreviation review${pendingAbbreviations === 1 ? '' : 's'} and ${pendingFindings} clinical finding${pendingFindings === 1 ? '' : 's'} still require physician attention.`}</p><small>Invalid or incomplete findings cannot appear ready for patient memory.</small></SectionCard>
}
