...
describe('shared physician workflow', () => {
  it('starts processing from upload and preserves patient, encounter, and document context', async () => {
    ...
    expect(
      await screen.findByText('Document ready to process'),
    ).toBeInTheDocument()
    ...
    expect(screen.getByText('enc_2026_0817_01')).toBeInTheDocument()
    expect(screen.getByText('doc_5521')).toBeInTheDocument()

    expect(
      screen.getByRole('heading', {
        name: 'Abbreviation review',
      }),
    ).toBeInTheDocument()
  })
  ...
  it('keeps patient memory on the default tab with exactly four tabs', async () => {
    ...
    expect(
      await screen.findByText('Relevant patient context'),
    ).toBeInTheDocument()

    expect(
      screen.getByText('Recent patient activity'),
    ).toBeInTheDocument()
  })
  ...
  it('does not expose the memory handoff until review and safety are complete', async () => {
    ...
    expect(
      await screen.findByText(
        'Complete abbreviation review before clinical findings can be generated.',
      ),
    ).toBeInTheDocument()

    expect(
      screen.queryByRole('button', {
        name: 'Open patient memory',
      }),
    ).not.toBeInTheDocument()
    ...
    expect(
      screen.getByDisplayValue('pat_00123'),
    ).toBeInTheDocument()

    expect(
      screen.getByDisplayValue('enc_2026_0817_01'),
    ).toBeInTheDocument()
  })
})