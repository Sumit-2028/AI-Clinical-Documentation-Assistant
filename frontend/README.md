...
Current integration status: production-mode adapters now call the FastAPI
gateway through `src/api/client.ts`; deterministic fixtures remain available
only for tests. See the repository-level [frontend/backend integration
document](../docs/frontend-backend-integration.md) for the verified endpoint
mapping, commands, and limitations. The historical planning notes below are
...
The frontend reads `VITE_API_BASE_URL` from `frontend/.env.example`; the
default is `http://127.0.0.1:8000`. Do not put backend secrets or AI provider
